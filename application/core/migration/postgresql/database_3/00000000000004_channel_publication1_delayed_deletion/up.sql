CREATE TABLE public.channel_publication1_delayed_deletion (
    channel_publication1__id BIGINT,
    can_be_deleted_from BIGINT,
    created_at BIGINT
) WITH (oids = false, fillfactor = 90, autovacuum_enabled = true);

CREATE INDEX channel_publication1_delayed_deletion_1 ON public.channel_publication1_delayed_deletion
USING btree (can_be_deleted_from ASC NULLS LAST) WITH (fillfactor = 90, deduplicate_items = on);

ALTER TABLE ONLY public.channel_publication1_delayed_deletion
