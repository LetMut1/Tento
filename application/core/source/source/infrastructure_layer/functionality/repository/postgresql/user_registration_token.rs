...
impl Repository<Postgresql<UserRegistrationToken<'_>>> {
    ...
    pub fn find_1<'a>(database_2_client: &'a Client, by_1: By1<'a>) -> impl Future<Output = Result<Option<UserRegistrationToken_1>, AggregateError>> + Send + use<'a> {
        return async move {
            let query = "\
                SELECT \
                    urt.value AS v,\
    ...
    pub fn find_2<'a>(database_2_client: &'a Client, by_1: By1<'a>) -> impl Future<Output = Result<Option<UserRegistrationToken_2>, AggregateError>> + Send + use<'a> {
        return async move {
            let query = "\
                SELECT \
                    urt.value AS v,\
    ...
    pub fn find_3<'a>(database_2_client: &'a Client, by_1: By1<'a>) -> impl Future<Output = Result<Option<UserRegistrationToken_3>, AggregateError>> + Send + use<'a> {
        return async move {
            let query = "\
                SELECT \
                    urt.value AS v,\
