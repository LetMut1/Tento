use {
    ...
    dedicated::{
        action_processor_incoming_outcoming::action_processor::user::send_email_for_reset_password::{
            Incoming,
            Outcoming,
            Precedent,
        },
        ...
        return async move {
            ...
            TaskSpawner::spawn_tokio_non_blocking_task_into_background_repeatable(
                ...
                move || -> _ {
                    ...
                    let user_reset_password_token__value_ = user_reset_password_token__value.clone();
                    let user_device__id_ = user_device__id.clone();
                    return async move {
                        EmailSender::<UserResetPasswordToken>::send(
                            email_server,
                            user_reset_password_token__value_.as_str(),
                            user__email_.as_str(),
