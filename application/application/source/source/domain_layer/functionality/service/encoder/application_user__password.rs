...
impl Encoder<ApplicationUser_Password> {
    pub fn encode<'a>(application_user_password: &'a ApplicationUser_Password) -> Result<String, Auditor<Error>> {
        return Ok(Encoder_::<Argon2Id>::encode(application_user_password.0.as_bytes())?);
    }
