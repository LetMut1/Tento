...
impl Encoder<ApplicationUser_Password> {
    pub fn encode<'a>(application_user_password: &'a str) -> Result<String, AggregateError> {
        return Encoder_::<Argon2Id>::encode(application_user_password.as_bytes());
    }
    pub fn is_valid<'a>(application_user_password: &'a str, application_user__password_hash: &'a str) -> Result<bool, AggregateError> {
