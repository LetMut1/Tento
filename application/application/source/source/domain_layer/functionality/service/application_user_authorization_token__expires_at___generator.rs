...
impl Generator<ApplicationUserAuthorizationToken_ExpiresAt> {
    pub fn generate() -> Result<ApplicationUserAuthorizationToken_ExpiresAt, ErrorAuditor> {
        let application_user_authorization_token_expires_at = match Resolver::<DateTime>::unixtime_add_minutes_interval_from_now(ApplicationUserAuthorizationToken::QUANTITY_OF_MINUTES_FOR_EXPIRATION) {
