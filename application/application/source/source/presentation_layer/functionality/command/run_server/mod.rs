...
// enum ApplicationUser_Aggregator {  и подобные заменить просто на тапл необходимых полей. Сделать это после того, как сделаю задачу ниже, потому что сущности нужны были для апдкйта.
// subject: &'a T, Getter заменить просто на конкретные структуры. Чтобы не реализовывать постоянно трейт или чтобы можно было соьирать по кускам то, что нужно обновить.
// Может. вообще уйти от классического представления работы с сущностью? То есть, есть приватная Ентити. Все снрвисы работают со своими дто, в которых поля исходной сущности. Инсерт, например, не отдает полный объект, а отдает что?
// !1111111 то есть, Entity - это как просто dto. будут публичные поля, нет геттеров и сеттеров. NewType сделать pub fields. в репозиториях делать референс без промежуточного копирования Copy типов.
// Перенести и переименовать все инсерты в общий репозиторий. Insert1...2.
