...
impl Base {
    pub fn handle(aggregate_connection_pool: Arc<AggregateConnectionPool>, base_request: RequestBase) -> Result<ResponseBase, BaseError> {
        ...
        return Ok(
            ResponseBase::new(
                ApplicationUserPostgresql::is_exist_by_email(connection, &application_user_email)?
                || ApplicationUserPreConfirmedPostgesql::is_exist_by_application_user_email(connection, &application_user_email)?
            )
        );
    }
}