...
impl Base {
    pub fn handle(aggregate_connection_pool: Arc<AggregateConnectionPool>, request_base: RequestBase) -> Result<(), BaseError> {     // TODO Защита от частого посыла емэй
        let connection: &'_ mut Connection = &mut *ConnectionExtractor::get_redis_connection(&aggregate_connection_pool)?;

        if let Some(application_user_reset_password_token) = StateManagerApplicationUserResetPasswordTokenRedis::get_by_application_user_id(
            connection, &ApplicationUserId::new(request_base.get_application_user_id())
        )? 
        {
