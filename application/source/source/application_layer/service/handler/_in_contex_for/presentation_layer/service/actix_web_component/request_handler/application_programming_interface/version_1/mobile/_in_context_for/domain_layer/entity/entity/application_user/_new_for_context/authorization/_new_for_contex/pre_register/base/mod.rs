...
use crate::presentation_layer::data_transfer_object::request::_in_context_for::presentation_layer::service::actix_web_component::request_handler::application_programming_interface::version_1::mobile::_in_context_for::domain_layer::entity::entity::application_user::_new_for_context::authorization::_new_for_context::pre_register::base::Base as RequestBase;
...
impl Base {
    pub fn handle(aggregate_connection_pool: Arc<AggregateConnectionPool>, request_base: RequestBase) -> Result<(), BaseError> {
        let application_user_email: Email = Email::new(request_base.get_application_user_email());
        if ApplicationUserComponentValidator::is_valid_email(&application_user_email)? {
            ...
            if !ApplicationUserPreConfirmedPostgesql::is_exist_by_application_user_email(postgresql_connection, &application_user_email)? {
                if !ApplicationUserPostgresql::is_exist_by_email(postgresql_connection, &application_user_email)? {
                    ...
                    ApplicationUserRegistrationConfirmationTokenRedis::create(
                        &mut *ConnectionExtractor::get_redis_connection(&aggregate_connection_pool)?, &application_user_registration_confirmation_token
                    )?;

                    ApplicationUserPreConfirmedPostgesql::create(postgresql_connection, &application_user_pre_confirmed)?;
