...
impl DateTimeResolver {                     // TODO переделать дженерик на конкретный тип после ухода от прокседТАйпс
    ...
    pub fn is_greater_or_equal_than<'outer_a, Tz>(
        subject_date_time: &'outer_a DateTime<Tz>,
        than_date_time: &'outer_a DateTime<Tz>
    ) -> bool
    where
    ...
    ) -> Result<String, BaseError> {     // TODO Переделать, тут должно быть два параметрв. а Now  подставляем уже в другом сервисе
