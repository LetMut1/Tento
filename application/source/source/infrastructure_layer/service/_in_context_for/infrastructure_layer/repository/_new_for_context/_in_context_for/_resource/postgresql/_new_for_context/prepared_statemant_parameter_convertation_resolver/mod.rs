...
impl<'outer_a> PreparedStatementParameterConvertationResolver<'outer_a> {
    ...
    pub fn add_parameter<'this>(
        &'this mut self,
        parameter_value: &'outer_a (dyn ToSql + Sync),
        patameter_type: Type
    ) -> &'this mut Self {
    ...
    pub fn get_parameter_registry<'this>(
        &'this self
    ) -> &'this Vec<&'outer_a (dyn ToSql + Sync)> {
        return &self.parameter_registry;
    }

    pub fn get_parameter_type_registry<'this>(
        &'this self
    ) -> &'this Vec<Type> {
