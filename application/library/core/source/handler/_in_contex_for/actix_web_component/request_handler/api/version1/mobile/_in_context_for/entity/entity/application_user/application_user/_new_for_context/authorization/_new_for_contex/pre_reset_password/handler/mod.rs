...
use crate::dto::response_parameters::_in_context_for::handler::_in_context_for::actix_web_component::request_handler::api::version1::mobile::_in_context_for::entity::entity::application_user::application_user::_new_for_context::authorization::_new_for_context::pre_reset_password::handler::_new_for_context::result::Result as HandlerResult;
    ...
    pub fn handle(request: Request) -> Result<HandlerResult, MainErrorKind> {
        ...
        if let Some(application_user) = ApplicationUserBaseRepository::get_by_email(&connection_manager, &Email::new(request.application_user_email))? {
            ...
            match ResetPasswordTokenBaseRepository::get_by_application_user_id(&connection_manager, application_user.get_id())? {
                ...
                None => {
                    reset_password_token = ResetPasswordToken::new(&application_user);
