...
impl<'outer> Handler {
    pub fn handle(json_access_web_token: &'outer JsonAccessWebToken<'outer>) -> Result<(), MainErrorKind> {
        ...
        {
            JsonRefreshWebTokenBaseRepository::delete(&connection_manager, &json_refresh_web_token)?;  // TODO без транзакции, так как все будет на кеше (Удалить это сообщение, как только перепишу на Кеш)
