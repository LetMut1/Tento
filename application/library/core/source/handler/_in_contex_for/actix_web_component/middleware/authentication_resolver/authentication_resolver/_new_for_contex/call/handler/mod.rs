            ...
            if let Ok(header_value) = header_value.to_str() {
                if let Ok(json_access_web_token) = SerializationFormResolver::deserialize(header_value) {
                    if !json_access_web_token.is_expired() {
                        ...
                        return Err(EntityErrorKind::JsonAccessWebTokenErrorKind(JsonAccessWebTokenErrorKind::InJsonAccessWebTokenBlackList))?;
                    }

                    return Err(EntityErrorKind::JsonAccessWebTokenErrorKind(JsonAccessWebTokenErrorKind::AlreadyExpired))?;
                }
            }
        }
