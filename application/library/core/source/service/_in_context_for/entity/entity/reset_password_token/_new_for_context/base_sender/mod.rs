...
impl<'outer> BaseSender {
    pub fn send_by_email(reset_password_token: &'outer ResetPasswordToken<'outer>) -> Result<(), EmailErrorKind> {
        EmailSender::send(
            "Reset password token",
            "Your code: ".to_string() + reset_password_token.get_value().get_value(), 
            reset_password_token.get_application_user_email().get_value()
        )?;
