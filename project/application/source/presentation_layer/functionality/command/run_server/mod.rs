...
// Поменять названия апи. Вынести в константы. Убрать mobile
...
// Get_many_by_name - Фронт делает два запроса (или же я делаю 2 запроса. Лучше я) на поиск по имени. Один поиск идет по публичным каналам. Второй идет по тем, на которые ты подписан.

// bytes -> Incoming В один метод в сериалайзер.