...
// Возможно, отдавать с репозиториев картеж типов, чтобы не привязввать метод репозитория к слою аппликации. ЛИБО ЖЕ !! все типы перенести под запрос в репозиторий, чтобы можно было переиспользовать.
// Не забирать лишние поля. Лучше, написать запрос на конкретные поля. Переписать запросы, где забираются дополнительные ненжные данные? Там, где апдейт и инсерт, использовать сущность. Но в других перепписать.
