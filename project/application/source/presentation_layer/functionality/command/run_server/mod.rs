...
// Все unixtime, где неважно не попасть в десятую долю секунды, делать через код, а не внутри БД.

// can_send -> is_expired. ПОменять логику.

// Сделать ПропертиГенератор

// WrongEnterTriesQuantityIncrementor -> переписать на простой инкремент с проверко.

// Rust wrapping cost. Может, быть сделать вреппинг для свойств сущности, чтобы по типам для них потом имплементировать что-то.