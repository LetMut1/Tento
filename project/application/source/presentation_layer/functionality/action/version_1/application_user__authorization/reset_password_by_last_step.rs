...
use crate::application_layer::functionality::service::action_inner_processor::application_user__authorization::reset_password_by_last_step::ActionProcessor;
use crate::application_layer::functionality::service::action_inner_processor::application_user__authorization::reset_password_by_last_step::Incoming;
    ...
    <<T as MakeTlsConnect<Socket>>::TlsConnect as TlsConnect<Socket>>::Future: Send
{
    ...
    let action_processor_result = match ActionProcessor::process(
        database_1_postgresql_connection_pool, database_2_postgresql_connection_pool, incoming
    ).await {
    ...
    match action_processor_result_ {
        ...
        ActionProcessorResult::UserWorkflowPrecedent { user_workflow_precedent } => {
            match user_workflow_precedent {
                ...
                UserWorkflowPrecedent::ApplicationUserResetPasswordToken_IsNotApproved => {
                    let data = match Serializer::<MessagePack>::serialize(
                        &UnifiedReport::<Void>::communication_code(
                            CommunicationCodeRegistry::APPLICATION_USER_RESET_PASSWORD_TOKEN__IS_NOT_APPROVED
                        )
                    ) {
                        Ok(data_) => data_,
                        Err(error) => {
