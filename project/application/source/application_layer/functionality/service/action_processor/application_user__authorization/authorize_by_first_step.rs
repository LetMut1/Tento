...
impl ActionProcessor {
        ...
        <<T as MakeTlsConnect<Socket>>::TlsConnect as TlsConnect<Socket>>::Future: Send
    {
        ...
        let is_valid = match ApplicationUser_PasswordHashResolver::is_valid(
            ...
            application_user_aggregator.get_application_user_password_hash()
        ) {
        ...
        let (application_user_authorization_token_, can_send) = match application_user_authorization_token {
            ...
            None => {
                ...
                let insert = Insert {
                    application_user_id: application_user_aggregator.get_application_user_id(),
        ...
        if can_send {
            if let Err(mut error) = ApplicationUser_EmailSender::send_application_user_authorization_token(
                ...
                application_user_aggregator.get_application_user_email(),
...
enum ApplicationUser_Aggregator<'a> {
    First {
        application_user: ApplicationUser_1<'a>
    },
    Second {
        application_user: ApplicationUser_2<'a>
    }
}

impl<'a> ApplicationUser_Aggregator<'a> {
    pub fn get_application_user_id<'b>(&'b self) -> i64 {
        return match *self {
            Self::First { ref application_user } => {
                application_user.get_id()
            },
            Self::Second { ref application_user } => {
                application_user.get_id()
            }
        }
    }
    ...
    pub fn get_application_user_password_hash<'b>(&'b self) -> &'b str {
        return match *self {
            Self::First { ref application_user } => {
