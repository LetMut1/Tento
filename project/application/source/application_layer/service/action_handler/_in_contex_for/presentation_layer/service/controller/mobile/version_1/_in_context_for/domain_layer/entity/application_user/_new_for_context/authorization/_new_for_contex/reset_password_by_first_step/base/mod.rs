...
use crate::infrastructure_layer::error::error_auditor::_component::logic_error::LogicError;
    ...
    pub async fn handle(
        ...
        if let Some(application_user) = ApplicationUserDataProviderPostgresql::find_by_email(
            ...
            match application_user.get_id() {
                ...
                None => {
                    return Err(ErrorAuditor::LogicError {logic_error: LogicError::new(false, "Application_user_id should exist")});
                }
            }
