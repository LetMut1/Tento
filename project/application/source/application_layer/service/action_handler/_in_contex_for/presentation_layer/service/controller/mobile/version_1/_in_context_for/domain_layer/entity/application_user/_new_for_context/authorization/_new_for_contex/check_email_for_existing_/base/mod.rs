...
use crate::presentation_layer::data_transfer_object::request_data::_in_context_for::presentation_layer::service::controller::mobile::version_1::_in_context_for::domain_layer::entity::application_user::_new_for_context::authorization::_new_for_context::check_email_for_existing_::base::Base as RequestData;
...
impl Base {
        ...
        <<T as MakeTlsConnect<Socket>>::TlsConnect as TlsConnect<Socket>>::Future: Send
    {
        ...
        let response = Authorization::check_email_for_existing(environment_variable_resolver, request, postgresql_connection_pool).await;
        ...
        if response_parts.status == StatusCode::OK {
            match to_bytes(body).await {
                Ok(bytes) => {
                    match rmp_serde::from_read_ref::<'_, [u8], EndpointResponse<ResponseDataCheckEmailForExisting>>(bytes.chunk()) {
