...
impl Base {
    pub async fn handle<'a>(
        ...
        let response = Authorization::refresh_json_access_web_token(environment_variable_resolver, request, redis_connection_pool).await;
        ...
        if response_parts.status == StatusCode::OK {
            match to_bytes(body).await {
                Ok(bytes) => {
                    match rmp_serde::from_read_ref::<'_, [u8], EndpointResponse<ResponseDataRefreshJsonAccessWebToken>>(bytes.chunk()) {
