...
impl SerializationFormResolver<ApplicationUserAccessToken<'_>> {
    ...
    pub fn deserialize<'a>(
        ...
        let token_part_registry = application_user_access_token_serialized_form
        ...
        if token_part_registry.len() != 2
            || !Encoder::<Signature>::is_valid(
                pushable_environment_configuration,
                token_part_registry[0],
                token_part_registry[1],
            )
        {
            return Ok(
                InvalidArgumentResult::InvalidArgument {
                    invalid_argument: InvalidArgument::ApplicationUserAccessToken_DeserializedForm,
                },
            );
        }
