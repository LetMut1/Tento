...
impl Base {
    pub async fn create<'a>(
        ...
        application_user_access_token_black_list: &'a ApplicationUserAccessTokenBlackList<'_>
    ) -> Result<(), ErrorAuditor> {
        if let Err(error) = connection.set_ex::<String, u8, ()>(
            StorageKeyResolver::get_4(application_user_access_token_black_list.get_application_user_access_token_id()),
            1,
            (ApplicationUserAccessToken::QUANTITY_OF_MINUTES_FOR_EXPIRATION as usize) * (60 as usize)
        ).await {
            return Err(
                ErrorAuditor::new(
