...
impl ApplicationUserResetPasswordToken_PostgresqlRepository {
    pub async fn create<'a>(database_2_connection: &'a Connection, insert: Insert) -> Result<ApplicationUserResetPasswordToken, ErrorAuditor> {
        ...
        if row_registry.is_empty() {
            return Err(
                ErrorAuditor::new(
                    BaseError::LogicError { logic_error: LogicError::new("ApplicationUserResetPasswordToken can not be inserted into Postgresql database.") },
                    BacktracePart::new(line!(), file!(), None)
                )
            );
        }
    ...
    pub async fn update<'a>(
        ...
        if update.application_user_reset_password_token_expires_at {
            ...
            if row_registry.is_empty() {
                return Err(
                    ErrorAuditor::new(
                        BaseError::LogicError { logic_error: LogicError::new("ApplicationUserResetPasswordToken can not be updated in Postgresql database.") },
                        BacktracePart::new(line!(), file!(), None)
                    )
                );
            }
            ...
            WHERE aurpt.application_user_id = $4 \
            RETURNING \
    ...
    pub async fn delete<'a>(database_2_connection: &'a Connection, application_user_id: i64) -> Result<(), ErrorAuditor> {
        let mut prepared_statemant_parameter_convertation_resolver = PreparedStatementParameterConvertationResolver::new();
