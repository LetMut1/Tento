...
impl PostgresqlRepository<ApplicationUserAccessRefreshToken<'_>> {
    ...
    pub async fn delete_2<'a>(database_2_connection: &'a Connection, application_user_id: ApplicationUser_Id) -> Result<(), ErrorAuditor> {
        let application_user_id_ = application_user_id.get();
    ...
    pub async fn find_1<'a>(
        ...
        let application_user_access_refresh_token_obfuscation_value = match row_registry[0].try_get::<'_, usize, String>(1) {
            Ok(application_user_access_refresh_token_obfuscation_value_) => ApplicationUserAccessRefreshToken_ObfuscationValue::new(
        ...
        let application_user_access_refresh_token_expires_at = match row_registry[0].try_get::<'_, usize, i64>(2) {
            Ok(application_user_access_refresh_token_expires_at_) => ApplicationUserAccessRefreshToken_ExpiresAt::new(
                application_user_access_refresh_token_expires_at_
            ),
            Err(error) => {
                return Err(
                    ErrorAuditor::new(
        ...
        let application_user_access_refresh_token_updated_at = match row_registry[0].try_get::<'_, usize, i64>(3) {
            Ok(application_user_access_refresh_token_updated_at_) => ApplicationUserAccessRefreshToken_UpdatedAt::new(
                application_user_access_refresh_token_updated_at_
            ),
            Err(error) => {
                return Err(
                    ErrorAuditor::new(
