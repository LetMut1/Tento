...
impl<'b> Handler{
    pub fn handle(request: &'b Request) -> ReturnedType {        // TODO Всплывание ошибок, В РекуестХэндлере делать try. 
        ...
        let mut application_user_base_repository: ApplicationUserBaseRepository<'_> = 
            ApplicationUserBaseRepository::new(&pg_connection_manager);
        let application_user_existing: ApplicationUserExisting = application_user_base_repository.get_by_email(request.get_email());
        let application_user: ApplicationUser<'_> = ApplicationUser::new_from_model(&application_user_existing);
        if password_encoder.is_valid(request.get_password(), application_user.get_passord_hash().get_value()) {
            if application_user.is_confirmed() {
                ...
                let json_refresh_web_token_base_repository: JsonRefreshWebTokenBaseRepository <'_> =
                    JsonRefreshWebTokenBaseRepository::new(&pg_connection_manager);
