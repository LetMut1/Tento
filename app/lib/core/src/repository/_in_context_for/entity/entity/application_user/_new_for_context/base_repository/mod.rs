...
impl<'outer> BaseRepository {
    ...
    pub fn get_by_email(connection_manager: &'outer ConnectionManager, email: &'outer String) -> Result<Existing, DieselErrorKind> {
        match application_user::table.filter(application_user::email.eq(email)).limit(1).load::<Existing>(connection_manager.get_connection()) { // TODO если вернется ноль значений, то что делать
