...
impl<'b: 'c, 'c> SerializationFormResolver {
    const LINE_SEPARATOR: &'static str = ".";

    pub fn serialize(json_access_web_token: &'b JsonAccessWebToken<'b>) -> String {
    ...
    pub fn deserialize(jawt_classic_form: &'b String) -> PayloadCommon<'c> {
        ...
        if Self::is_valid(&jawt_parts) {
            ...
            return serde_json::from_slice::<'_, PayloadCommon<'c>>(paylod_json_encoded).unwrap();  // TODO По сути, обработать ошвозможную ошибку нужно, но ее не будет по факту
        } else {
            panic!("Выбрасываем исключение, то есть, возвращаем Резалт с кастомной ошибкой");   // TODO 
        }
    }

    fn create_classic_form(header: &'c String, payload: &'c String) -> String {
    ...
    fn is_valid(jawt_parts: &'c Vec<String>) -> bool {
        if jawt_parts.len() == 3 {
