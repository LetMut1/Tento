...
impl<'a, 'b: 'a> SerializationFormResolver {
    ...
    pub fn deserialize(jawt_classic_form: &'b String) -> PayloadCommon<'a> {
        ...
        if Self::is_valid(&jawt_parts) {
            ...
            return serde_json::from_slice::<'_, PayloadCommon<'a>>(paylod_json_encoded).unwrap();  // TODO По сути, обработать ошвозможную ошибку нужно, но ее не будет по факту
        } else {
            panic!("Выбрасываем исключение, то есть, возвращаем Резалт с кастомной ошибкой");   // TODO 
        }
    }

    fn create_classic_form(header: &'a String, payload: &'a String) -> String {
    ...
    fn is_valid(jawt_parts: &'a Vec<String>) -> bool {
        if jawt_parts.len() == 3 {
