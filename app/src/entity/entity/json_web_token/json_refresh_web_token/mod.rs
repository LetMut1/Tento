...
use self::core::value_hash::ValueHash;
...
pub struct JsonRefreshWebToken<'a> {
    ...
    value_hash: ValueHash<'a>,
    created_at: DateTime<'a>,
    expired_at: DateTime<'a>,
}

impl<'a> JsonRefreshWebToken<'a> { // TODO  create ValHas with CustomHasher
    pub fn new(user_id: &'a UuidV4, device_id: String, value: String) -> Self {
        return Self {
            ...
            value_hash: ValueHash::new(MaybeOwned::Owned(value)),
    ...
    pub fn from_model(model: &'a Existing) -> Self {
        return Self {
            ...
            value_hash: ValueHash::new(MaybeOwned::Borrowed(model.get_value_hash())),
    ...
    pub fn set_value(&'a mut self, value: String) -> &'a mut Self {
        self.value_hash = ValueHash::new(MaybeOwned::Owned(value));
    ...
    pub fn get_value_hash(&'a self) -> &'a ValueHash {
