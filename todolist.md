...
так же встраивать в токен информацию о подписке (
  !!!!!!!!!!!  либо удалит из Чаннел репо find_3  8 параметр, определяющий подписку
)
...
обновлять channel_token__is_channel_subscription_exist при подписке - отписке





в запросах на забор каналв и в создании канала нужно отдавать ChnnaelTokenSigned

channel__id: i64, НЕ ДОЛЖен Передаваться 1. Должен передаваться ТокенКанала - в какие ТокенСигнед можно положить, чтобы не брать из Бд для проверки существования канала.



const CT:  замениьь на конфиг. проверить, везде ли правильные конфиги используются.
Есть ошибки. Проверить