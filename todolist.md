...
убрать дто Инсерты и апдейты и заменить на деривативы, передаваемые через ссылку, и возврат должен быть только того значения, которое вернулось из бд, но не всей сущности целиком




fn update_1 ->>>>> update, если нет других апдейтов.
pub struct UserAccessRefreshToken<'a> { - не должно остаться лайфтайма. Все структуры - строго описательрые. Сделать приватными поля. ВОзможно, со
Проверить, что в репозитории нигде не возвращается что-то, что пришло в аргументы.
убрать entity::new + pub -> pivate fields
...
После того, как сделаю то. что выше, посмотреть, нужно ли иметь сущности в dedicated. Надо ли их перенести.



insert: &'a Insert1  - не должно быть. Здесь именно овнермувинг. Проверить для Апдейт и Бай.
