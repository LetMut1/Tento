...
// C разных роутов генерировать разное время expires_at для токенов. ???
...
На сервере есть огромное количество задач подсчета хеша. Есть ощущение, что для этих задач нужно создать отдельный пул rayon-сридов,
но здесь нужно понять точнее, эффект от Района имеется, когда нужно посчитать сразу много хешей, или же он с одним хешем справляется быстрее.
Скорее всего, для async io хватит несколько ТокиоНонБлокинг сридов, а остальные сриды нужно оставить под CpU-bound задачи.
При текущй реализации, когда все хеши считаются на Токио Блокинг сридах, есть вероятность того, что задачи подсчта хеша не будут давать выполняться асинку, так
как постоянно нужно будет считать следующий хеш и так далее.
TOKIO/RAYON---------------------------------------------------------------------
...
// Сделать так, чтобы нельзя было очень часто рефрешить токены. Возможно, просто поставить там проверку на то, сколько времени точно нельзя рефрещить токены - 20 минут после рефреша, если экспирация в 30 минут.
...
убрать в Спавнере типы Тасков и сделать просто 4 метода.



делать Енкод и валидацию токенов на НЕВоркерСРидахТокие (Блокинг)
Сначала копировать в основной процесс, затем овнермувинг в воркерСрид. Но проблема будет с НеКопи типами.
