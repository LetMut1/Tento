...
неверная десериализация аксесс токена должна отдавать 400, а не 500


в тестах аллокации нужно сделать так, чтобы проверялось количества вариантов матчинга Прецедентов. То есть, сделать матчинг


Защита от подписки с переблрм - страница канала отдает 1 токен на день. - только с токеном с get_one-by-id

защита от взятия(нахождения) приватного канала по get_one_by_id роботом ? только с хешем от get_many_by_name