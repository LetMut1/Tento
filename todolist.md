...
// !- Все лайфтаймы для Fn/FnOnce/FnMut, в сигнатуре которых есть лайфтаймы, должны быть объявлены через HRTB (for<'a>), если обратного не требует конкретный случай. Сейчас есть баг https://github.com/rust-lang/rust/issues/102211#issuecomment-1513931928 , но, скорее всего, его уже скоро закроют.
...
запрашивать из постгрес номер Последовательности и потом на него регистрировать юзера - посмотреть, где можно оптимизировать еще.

все последовательности постгреса начать с отрицательного числа

посмомтреть, где есть долгие процессорные операции, и вернуть коннекшн в пул перед ними.
...
реализовать генераторы значений для токено, а постоянный 666 сделать через фичи.
...
лайки к публикациям индексировать по channel_publication__created_at, чтобы запрашивать лайки по мере прокрутки ленты.


Пока что публикации сделать влоб. Но затем, если прибавится количество разновидностей публикаций есть, 2 варианта с разными преиммуществами:
1.делать таблицу под каждый тип публикации, запрашивать иъ с фронта одновременно и сортировать по времени создания, либо в этом варианте иметь общую таблицу публикаций с айдишниками на конкретную.
2.сделать общую таблицу с флагом ТипПубликации, и саму публикацию хранить как байты под этот тип - здесь невозможно будет ссылать на эту публикацию ддя других таблиц.
Начную реализацию изначально с варианта 1.


в бд сохранять created_at в миллисекундах там?



проверить, что в репозитории все ппараметры заходятя через prepared_type, но не в создагнии строки, иначе будут плодиться препареды тейтманты.