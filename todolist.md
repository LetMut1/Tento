...
Начну реализацию изначально с варианта 1.
...
Запрашивать публикации через ЛефтДжоин с лайками.


Убрать ненужную проверку на существование и сделать через конфликт с количеством строк.
Сделать по аналогии с Repository<Postgresql<ChannelPublication1Mark>>::create()



при лайке понимать, что существует пуликация через токен, но не через запрос в бд. При таком способе можно лайкнуть публикацию, котоурю уже удалили.
Как удалять такие лайки?
Сначала помечать публикацию как удаленную, через фиксированное время удалять пцбликацию. а до этого удалять все лайки. Время жизни токена должно быть менше времени ожидания удаления, чтобы все токены эксперировались