...
проверить в репозитории, какие запросы стали не нужныю.
...
возможно, добавить channel__access_modifier, так как он проверяется на заборе публикаций



incoming.limit - все u8 должны быть


channel_subscription routes. Провекра на овнера. Заменить
...
Validator::<Channel_Id>::is_valid  Будет не нужен после задачи выше.
find_7 Будет не нужен после задачи выше.

Нужен ли channel__id в ПубликейшнТокен. В видимых данных не нужен, но нужен внуьри подписи.,  Посмотреть с остальными айди в остальных токенах.
Где-то нужны, где-то не нужны. Если нужны, то проверяем айдишник из других токенов